...
///  Test stories
/// 
///  Test SimpleStake in isolation from DHashToken protocol
///  - 
...
contract('SimpleStake', function(accounts) {
	...
	/// mock DHashToken protocol contract address with an external account
	const DHashTokenProtocol = accounts[4];
	/// mock upgraded DHashToken protocol contract address with an external account
	const upgradedDHashTokenProtocol = accounts[5];
	...
	describe('in isolation', async () => {
		...
		before(async () => {
			var contracts =
				...
				artifacts, accounts, DHashTokenProtocol, UUID);
		...
		context("on construction", async () => {
			...
			it("should have a well-defined DHashTokenProtocol", async() => {
				// assert protocol contract address is set
				Assert.equal(await simpleStake.DHashTokenProtocol.call(), DHashTokenProtocol);
		...
		context('before protocol transfer', async () => {
			...
			it("can release 3ST", async () => {
				const result = await simpleStake.releaseTo(accounts[0], ST3, { from: DHashTokenProtocol });
				...
				SimpleStakeUtils.checkReleasedEventGroup(result, DHashTokenProtocol, accounts[0], ST3);
			});

			it("must fail to release 3ST", async () => {
				await Utils.expectThrow(simpleStake.releaseTo(accounts[0], ST3, { from: DHashTokenProtocol }));
				await SimpleStakeUtils.checkTotalStaked(simpleStake, token, ST2);
				Assert.equal((await token.balanceOf(accounts[0])).toNumber(), totalSupply.sub(ST2).toNumber());
			});
			...
			it("can release all remaining stake", async () => {
				...
				Assert.ok(await simpleStake.releaseTo(accounts[0], remainingStake, { from: DHashTokenProtocol }));
