...
//
// ----------------------------------------------------------------------------
// Utility chain: DHashTokenUtility
//
...
/// @title DHashToken Utility
contract DHashTokenUtility is Hasher, OpsManaged {
    ...
    /// @dev redeemer must set an allowance for the branded token with DHashTokenUtility
    ///      as the spender so that the branded token can be taken into escrow by DHashTokenUtility
    ///      note: for STPrime, call DHashTokenUtility.redeemSTPrime as a payable function
    ///      note: nonce must be queried from DHashTokenValue contract
    function redeem(
    	bytes32 _uuid,
    	uint256 _amountBT,
    	uint256 _nonce)
